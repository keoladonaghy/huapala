-- Validation System Database Schema for Neon PostgreSQL

-- Table to store validation sessions (batch processing runs)
CREATE TABLE validation_sessions (
    ...
    songs_flagged INTEGER DEFAULT 0,
...
CREATE TABLE song_validations (
    id SERIAL PRIMARY KEY,
    canonical_mele_id INTEGER REFERENCES canonical_mele(id) ON DELETE CASCADE,
...
JOIN song_validations sv ON cm.id = sv.canonical_mele_id
LEFT JOIN validation_issues vi ON sv.id = vi.song_validation_id
WHERE sv.manual_review_required = TRUE
GROUP BY cm.id, cm.canonical_title_hawaiian, cm.primary_composer, 
...
BEGIN
    ...
    FROM canonical_mele cm
    LEFT JOIN song_validations sv ON cm.id = sv.canonical_mele_id
    LEFT JOIN validation_issues vi ON sv.id = vi.song_validation_id
    WHERE cm.id = song_id

...
class SongModalManager {
    ...
    switchTab(tabName) {
        ...
        // Load songbooks data when the songbooks tab is clicked
        if (tabName === 'songbooks' && songPage.song) {
            this.loadSongbooksInfo(songPage.song.canonical_mele_id, document.getElementById('songbooksTab'));
        }
    }
    ...
    async loadSongbooksInfo(songId, container) {
        // Show loading state
        ...
        try {
            ...
            if (data.songbooks && data.songbooks.length > 0) {
                ...
                let html = `
                    <div class="songbooks-section">
                        <h3>Found in ${data.count} Songbook${data.count > 1 ? 's' : ''}</h3>
                        <div class="songbooks-list">
                `;
                
                data.songbooks.forEach(songbook => {
                    html += `
                        <div class="songbook-entry">
                            <div class="songbook-main">
                                <strong>${songbook.songbook_name}</strong>
                                ${songbook.pub_year ? ` (${songbook.pub_year})` : ''}
                            </div>
                            <div class="songbook-details">
                                ${songbook.page ? `<span class="songbook-page">Page ${songbook.page}</span>` : ''}
                                ${songbook.composer ? `<span class="songbook-composer">Composer: ${songbook.composer}</span>` : ''}
                            </div>
                            ${songbook.songbook_entry_title !== songbook.songbook_name ? 
                                `<div class="songbook-title">Listed as: "${songbook.songbook_entry_title}"</div>` : ''
                            }
                        </div>
                    `;
                });
                ...
                container.innerHTML = `
                    <div class="songbooks-section">
                        <h3>Songbook Information</h3>
                        <p class="songbooks-empty">
                            We currently do not have this song linked to a songbook in our database.
                        </p>
                        <p class="songbooks-note">
                            We are developing a means by which users can suggest links to books that contain this mele. Please check back later.
                        </p>
                    </div>
                `;
            }
            ...
            container.innerHTML = `
                <div class="modal-error">
                    Unable to load songbook information. Please try again later.
                </div>
            `;
        }
    }
